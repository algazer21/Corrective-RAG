...
def CRAG(query,corpus,answers,safeURLs,printp = True):
    """
    ...
    """
    #Calls the evaluator
    context,boo = evaluator(query,corpus,answers,printp)
    ...
    if boo:
        ...
        context = WebRequest(context,safeURLs,printp)
...
def WebRequest(query,safeURLs = [],printp = True):
    """
    ...
    if printp: print(f"Web Query: {queryW}")
    ...
    if safeURLs:
        if len(webSecureCorpus)>0:
            ...
            print("Error, no results found un the reliable URLs")
    if printp: print(textwrap.fill(webCorpus[:200]))
